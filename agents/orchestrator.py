...
class TeachingAgentOrchestrator:
    ...
    def __init__(self):
        self.state = {'slide_text': '', 'topic': '', 'lesson_plan': '', 'research_links': [], 'final_report': '', 'pdf_path': ''}
    ...
    def generate_lesson_plan(self, context_info: str='60 minutes, general audience'):
        ...
        messages = [{'role': 'system', 'content': f'You are an expert teaching assistant. Create a lesson plan based on the slides. Context: {context_info}. Include objectives, a timed outline, and one practical exercise.'}, {'role': 'user', 'content': f"Here is the text from the slides:\n{self.state['slide_text'][:3000]}\n\nPlease generate the lesson plan."}]
        ...
        topic_messages = [{'role': 'system', 'content': 'You are a precise keyword extractor. Extract a 1 to 3 word topic summary from the text. Return ONLY the 1-3 words, no introductory text, no quotes, no explanations.'}, {'role': 'user', 'content': f'Text: {plan[:1000]}\n\nKeywords only:'}]
